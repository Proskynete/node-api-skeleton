...
/**
 * K6 Performance Test - Greetings v2 API
...
export default function () {
  ...
  check(res, {
    ...
    "has timestamp property": (r) => {
      const body = JSON.parse(r.body);
      return body.hasOwnProperty("timestamp");
    },
    "has version property": (r) => {
      const body = JSON.parse(r.body);
    ...
    "timestamp is valid ISO": (r) => {
      const body = JSON.parse(r.body);
      return /^\d{4}-\d{2}-\d{2}T/.test(body.timestamp);
    },
...
function textSummary(data, options = {}) {
  ...
  let summary = `\n${indent}Greetings v2 Performance Test Summary\n`;
